...
- 제출 이후 대기하다가 조건이 만족되어 체결되면 `maker`
...
`realizedPnl = closeSidePnl - openFee - closeFee - fundingCost`

MVP 1차는 수수료와 funding을 포함한 순손익을 별도 필드로 함께 저장한다.
...
`positionRoi = accumulatedRealizedPnl / originalInitialMargin`
...
- `0 < 순실현손익 < 100 USDT`: `1 point`
- `100 <= 순실현손익 < 500 USDT`: `5 points`
- `500 <= 순실현손익 < 1000 USDT`: `20 points`
- `1000 USDT <= 순실현손익`: `50 points`

이 구간은 상점 아이템 가격이 정해지면 다시 조정할 수 있다.
