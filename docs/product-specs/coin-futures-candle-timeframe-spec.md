...
현재 저장소의 시장 히스토리 스키마는 원본 `market_candles_1m`과 completed 캔들 저장소 `market_completed_candles`를 기준으로 한다.
`market_completed_candles`는 DB-safe interval token으로 `1h`, `1D`, `1M` completed row를 함께 저장하며, 사용자 경험 관점의 전체 기간 목록은 이 저장 구조 위에서 직접 조회 또는 파생한다.
...
- DB에 물리 저장되는 권위 있는 캔들은 원본 `1m`와 completed 캔들이다.
- `1m`는 `market_candles_1m`의 원본 분봉 저장 기준이다.
- completed 캔들은 `market_completed_candles`에 저장하며, `candle_interval` DB token으로 `ONE_HOUR`, `ONE_DAY`, `ONE_MONTH`를 구분한다. API label은 각각 `1h`, `1D`, `1M`이며, `1W`는 저장된 `1D`를 주 경계로 파생한다.
- `market_completed_candles`에는 source audit 컬럼을 두지 않는다. V34 초기 적재는 기존 `market_candles_1h`를 `ONE_HOUR`로 복사한 뒤, 저장된 `ONE_HOUR.open_time`을 UTC 일/월 경계로 잘라 `ONE_DAY`/`ONE_MONTH` 초기 row를 SQL로 생성한다.
- API와 프론트 차트는 `1m`, `3m`, `5m`, `15m`, `1h`, `4h`, `12h`, `1D`, `1W`, `1M`을 지원 기간으로 고정한다.
- `3m`, `5m`, `15m`, `4h`, `12h`는 별도 물리 테이블을 기본값으로 두지 않고 저장된 `1m`/completed `1h` 캔들에서 파생한다.
- `1D`, `1M` REST DB read는 `market_completed_candles`의 `ONE_DAY`/`ONE_MONTH` row를 직접 조회하고, `1W`는 persisted `ONE_DAY` row를 주 경계로 파생한다. 일반 REST read path에서 매 요청마다 completed `1h`를 calendar rollup하지 않는다.

즉, 저장 구조는 "표시 가능한 전체 기간 목록"이 아니라 "파생 기간을 계산하기 위한 최소 저장 기준과 hot calendar completed row"이다.
...
- `1h`는 `market_completed_candles(candle_interval=ONE_HOUR)`에 저장되는 서버나 배치가 재사용할 수 있는 대표 롤업 저장 기준으로 유지한다.
...
| `1h`        | `market_completed_candles` (`ONE_HOUR`)  | 직접 조회      |
| `4h`        | `market_completed_candles` (`ONE_HOUR`)  | `1h` 묶음 롤업 |
| `12h`       | `market_completed_candles` (`ONE_HOUR`)  | `1h` 묶음 롤업 |
| `1D`        | `market_completed_candles` (`ONE_DAY`)   | 직접 조회      |
| `1W`        | `market_completed_candles` (`ONE_DAY`)   | `1D` 묶음 롤업 |
| `1M`        | `market_completed_candles` (`ONE_MONTH`) | 직접 조회      |

핵심은 "사용자 지원 기간 수"와 "DB 물리 테이블 수"를 1:1로 맞추지 않는 것이다.
현재 기준에서는 원본 `1m`와 completed `1h`/`1D`/`1M`만 물리 저장하고, 나머지는 애플리케이션 계층에서 파생하는 방식을 기본값으로 둔다.

REST history에서 `1h`는 `market_completed_candles(candle_interval=ONE_HOUR)`에 저장된 REST-visible completed row를 직접 조회한다. `1D`와 `1M`은 각각 `ONE_DAY`, `ONE_MONTH` completed row를 직접 조회하고, `1W`는 persisted `ONE_DAY` row를 주 경계로 롤업한다.
...
`ONE_HOUR` completed row는 저장 또는 재빌드 시점에 해당 hour의 `[hourOpen, hourClose)` 구간에 기대되는 모든 `1m` open time 60개가 연속으로 존재할 때만 저장된다.
...
기대 open time 중 하나라도 없으면 gap으로 보고 해당 hour의 `market_completed_candles(candle_interval=ONE_HOUR)` 저장 또는 재빌드를 거부하여 partial row가 REST-visible completed row로 남지 않게 한다.
...
`4h`, `12h`는 partial 후보가 아닌 completed `1h` 입력만 rollup해야 한다. `1W`는 completed `1D` 입력만 rollup해야 한다. `1D`, `1M`은 completed calendar row를 직접 읽는다.
Completed calendar candle에서 `1D`는 UTC 일 경계 `[00:00, 다음 00:00)`, `1W`는 UTC ISO 주 경계 `[월요일 00:00, 다음 월요일 00:00)`, `1M`은 UTC 월 경계 `[1일 00:00, 다음 달 1일 00:00)`를 따른다. `1D`/`1M` 초기 적재는 해당 시간 경계 안에 존재하는 completed `1h` row를 집계해 초기값을 만들고, `1W`는 저장된 `1D` row를 기준으로 요청 시 파생한다.
런타임 재빌드 경로는 새로 닫힌 `1h`를 기준으로 닫힌 calendar bucket을 다시 계산해 upsert할 수 있다.
...
월/주 단위 집계 비용이 hot path 비용이 되면 `market_completed_candles(candle_interval, open_time, symbol_id)` 인덱스 또는 별도 materialized metadata를 도입해 매 요청마다 전체 hourly row를 스캔하지 않게 한다.

### 후속 확장 원칙

특정 기간의 조회량이 커지거나 롤업 비용이 부담되면, 그때 `4h`, `12h` 같은 중간 interval을 `market_completed_candles`에 추가 물리화할 수 있다.
하지만 첫 원칙은 "필요 이상으로 테이블을 늘리지 않는다"이다.

### 오래된 구간 조회 기준

DB에 저장된 `1m`와 completed 캔들만으로 요청한 `limit`을 모두 채우지 못하면 서버는 부족한 오래된 구간만 캐시 경로에서 보충한다.
...
`1D`/`1M`은 persisted completed calendar row의 최신 open time을 interval별 cursor 기준으로 사용한다. `1W`는 persisted `1D` row의 최신 open time을 기준으로 최신 완료 주 버킷을 계산한다.
...
- 현재 시각 이후로 닫히는 진행 중 일/주/월 봉은 completed REST 응답에 섞지 않는다.
- REST `1h`와 그 이상 시간봉 파생 기간은 completed `1h` 입력 또는 persisted completed calendar row만 사용해야 하며, 현재 진행 중인 partial hourly row를 응답에 포함하면 안 된다.
...
- `1m`, `1h`, `1D`, `1M`은 DB에 저장된 원본 또는 completed 캔들 테이블에서 직접 조회되고, `1W`는 completed `1D` row에서 파생된다.
- `1h` REST 조회는 저장 시점에 contiguous `1m` coverage가 증명된 completed hourly row만 반환하며 조회 시점에 `1m` coverage를 다시 스캔하지 않는다.
- `3m`, `5m`, `15m`, `4h`, `12h`는 현재 저장 구조를 기준으로 올바르게 파생되어 응답되고, `1D`/`1M`은 persisted completed calendar row를 기준으로 응답되며, `1W`는 persisted `1D` row를 기준으로 응답된다.
...
- [backend/storage/src/main/resources/db/migration/V34\_\_generalize_completed_market_candles.sql](/Users/hj.park/projects/coin-zzickmock/backend/storage/src/main/resources/db/migration/V34__generalize_completed_market_candles.sql)
