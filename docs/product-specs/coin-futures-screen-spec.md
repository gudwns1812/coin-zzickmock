...
- Google onboarding에서 이름, 닉네임, 이메일, 휴대폰 번호, 주소, 상세주소, 약관/리스크 동의 체크를 입력
...
- 새 계정 시작 선택: 이름, 닉네임, 이메일, 휴대폰 번호, 우편번호, 주소, 상세 주소, 필수 동의
...
- 외부 시세 수집 실패 시에도 지원 심볼명, 로고, 설명, 화면 shell은 유지한다.
- 실패 상태를 별도 문구나 임의 대체값으로 강조하지 않고, 가격성 필드는 비워 둔다.
- fallback seed 가격을 실제 시세처럼 표시하거나 주문 기준으로 사용하지 않는다.
...
- 차트 영역과 우측 주문 패널 사이에는 compact 가격 선택 패널을 둘 수 있다. 이 패널은 실제 호가창이 아니라 빠른 Limit 가격 입력 shortcut이다.
...
- 가격 행은 최신 체결가 위 7개, 최신 체결가, 최신 체결가 아래 7개로 구성하고, `Ask +n` 또는 `Bid -n` 같은 행 라벨 없이 가격만 표시한다.
- 사용자는 가격 간격 단위를 드롭다운으로 선택할 수 있다. `BTCUSDT`는 `0.1`, `1`, `10`, `100`, `1000`, `ETHUSDT`는 `10`, `1`, `0.1`, `0.01` 단위를 제공한다. 선택한 단위는 브라우저 local storage에 심볼별로 저장되어 재진입 후에도 유지된다.
...
- 기존 포지션 레버리지는 레버리지 모달에서 `적용`을 누르는 순간 `/positions/leverage`로
  ...
  않는다. `ISOLATED`와 `CROSS` 모두 레버리지 변경은 같은 심볼과 같은 마진 모드의 LONG/SHORT
  포지션 전체에 적용한다. 대상 심볼에 미체결 open 주문이 있으면 레버리지 변경은 실패한다.
...
- 시세 stale 시 주문 버튼 비활성화 여부를 명확히 표시하고, 제출 시점에 10초 초과 stale 시세가 확인되면
  서버의 `MARKET_PRICE_STALE` 오류 메시지를 표시한다.
...
- 기본 리필 횟수는 KST 월요일 시작 주간 기준 1회다. 상점의 20P `ACCOUNT_REFILL_COUNT` 구매는 현재 주간 리필 가능 횟수에 1회를 더하며, 무료 횟수와 같은 카운터에 합산되고 다음 KST 월요일 00:00 리셋 때 이월되지 않는다.
...
- 인증되지 않은 사용자는 보호 라우트의 서버 렌더링 경계에서 `/login`으로 리다이렉트한다.
...
- 리필 횟수 추가권을 `ACCOUNT_REFILL_COUNT` 타입 상품으로 제공한다. 가격은 20P이고 구매 즉시 현재 KST 주간에 사용 가능한 지갑 리필 횟수가 1회 늘어난다.
...
- 모든 커뮤니티 라우트는 로그인 필요.
- 일반 사용자는 `사례분석`, `코인정보`, `잡담` 글만 작성/수정할 수 있다.
...
- 일반 글 목록은 최신순이며, `사례분석`/`코인정보`/`잡담` 카테고리 필터를 제공한다.
- 목록 상단에는 게시글 제목, 작성자, 카테고리 표시 텍스트를 대상으로 하는 기본 검색 입력을 제공한다.
