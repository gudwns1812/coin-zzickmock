...
10. Infra host 효과가 있는 배포 후에는 infra host 안에서 Prometheus와 Loki readiness를 확인한다. Backend host는 건드리지 않는다.
...
운영 backend compose는 Spring Boot actuator metrics를 별도 infra host Prometheus가 가져갈 수 있도록 backend container `8080`을 host `${BACKEND_BIND_ADDRESS:-0.0.0.0}:${BACKEND_PORT:-8080}`로 publish한다. Push server metrics는 push-app container `8081`을 host `${PUSH_BIND_ADDRESS:-127.0.0.1}:${PUSH_PORT:-8081}`로 publish한다. 이 경로들은 관측 도구 전용 direct/private access이며 Nginx를 경유하지 않는다.

- Scrape URL: `http://<EC2_BACKEND_METRICS_HOST>:8080/actuator/prometheus`
- Push-app scrape URL: `http://<EC2_BACKEND_METRICS_HOST>:8081/actuator/prometheus` if `PUSH_BIND_ADDRESS` allows infra-host reachability; otherwise Prometheus may scrape the in-compose `push-app:8081` target in local/single-host topology only.
