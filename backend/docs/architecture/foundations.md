...
- `app` Gradle module은 primary executable assembly root다. boot runtime, component scan, configuration, adapter wiring, user-facing write/read API를 소유한다.
- `push-app` Gradle module은 Redis Stream-backed push server executable이다. canonical SSE route relay, Redis Stream consumer group, short-lived event fan-out만 소유하고 DB/JPA/Flyway/external provider module을 의존하지 않는다.
...
Redis Stream consumer polling처럼 push server가 외부 runtime tick으로 delivery application 협력 객체를 깨우는 경우도 `job` trigger로 둔다.
...
- service 흐름을 흐리는 concrete 실행 절차/계산/조합은 `application/implement`에 둘 수 있음
...
- `application/implement/common`, `application/implement/util`, `application/implement/helper` 같은 하위 잡동사니 package
- `application/implement` class 이름에서 owning domain/use-case prefix를 생략하거나 `Manager`, `Helper`, `Util`, generic `Processor`로 책임을 숨기는 것
