...
- [ ] 검증: 관련 테스트와 `./gradlew architectureLint`는 통과, `./gradlew check`는 `JpaMarketHistoryRepositoryTest` 스위트 실패로 blocker
- [ ] 품질 점수 확인: 코드 관점은 pass 가능하나 `check` blocker 때문에 최종 통과로 닫지 않음
- [ ] PR 생성
...
- 관찰:
  `BackendBeanWiringTest`는 처음에는 풀 컨텍스트에서 스케줄러까지 켜고 올라와 market persistence 테스트와 간섭할 수 있었다.
  증거:
  [backend/src/main/java/coin/coinzzickmock/feature/market/application/realtime/MarketRealtimeFeed.java](/Users/hj.park/projects/coin-zzickmock/backend/src/main/java/coin/coinzzickmock/feature/market/application/realtime/MarketRealtimeFeed.java)에 `@Scheduled`와 `@PostConstruct`가 같이 있다.

- 관찰:
  `JpaMarketHistoryRepositoryTest`는 단독 실행과 일부 조합 실행에서는 통과하지만, 전체 `./gradlew check`에서는 `assertEquals(1, count("market_candles_1m"))`에서 실패한다.
  증거:
  `cd backend && ./gradlew test --tests coin.coinzzickmock.feature.market.infrastructure.persistence.JpaMarketHistoryRepositoryTest --console=plain` 는 성공했고,
  `cd backend && ./gradlew check --no-daemon --console=plain` 는 같은 테스트의 104번째 줄에서 실패했다.
...
- `BackendBeanWiringTest`에는 `spring.task.scheduling.enabled=false`를 추가해 bean wiring 확인만 하도록 범위를 좁혔다.
- 관련 focused test와 `./gradlew architectureLint`는 통과했다.
- 남은 blocker는 전체 `./gradlew check`에서 재현되는 `JpaMarketHistoryRepositoryTest` 스위트 실패 원인 확인이다.
