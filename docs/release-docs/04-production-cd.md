...
- `backend_runtime`: `docker-compose.backend.prod.yml` 변경 또는 수동 `backend-runtime`. 새 backend image를 만들지 않고 backend host compose/env preflight 후 backend만 recreate한다. Split compose migration 중 이전 `coin-zzickmock` backend container가 backend host port를 잡고 있으면 해당 expected stale backend container만 제거한 뒤 새 backend를 올릴 수 있다. Infra host는 건드리지 않는다.
