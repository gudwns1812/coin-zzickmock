...
<Why_This_Exists>
Complex tasks often fail silently: partial implementations get declared "done", tests get skipped, edge cases get
forgotten. Ralph prevents this by looping until work is genuinely complete, requiring fresh verification evidence before
allowing completion, and using tiered architect review to confirm quality.
</Why_This_Exists>
...
- When Serena MCP is available, use it as the default low-token discovery path for repository lookup, symbol search,
  reference tracing, and ambiguity reduction before broader shell exploration or full-file reads
- Use Serena's symbolic/pattern tools to minimize token usage: prefer candidate discovery + targeted verification over
  loading whole files
- Deliver the full implementation: no scope reduction, no partial completion, no deleting tests to make them pass
- Default to concise, evidence-dense progress and completion reporting unless the user or risk level requires more
  detail
- Treat newer user task updates as local overrides for the active workflow branch while preserving earlier
  non-conflicting constraints
- If correctness depends on additional inspection, retrieval, execution, or verification, keep using the relevant tools
  until the execution loop is grounded
- Continue through clear, low-risk, reversible next steps automatically; ask only when the next step is materially
  branching, destructive, or preference-dependent
  </Execution_Policy>

<Serena_Decision_Protocol>

- Apply this protocol whenever Ralph needs to choose among possible files, symbols, causes, implementations, or
  verification targets
- Step 1: Use Serena MCP first and gather **at least 3 candidates** when the search space permits
- Step 2: Compare the candidates explicitly against current evidence, constraints, and codebase context; keep short
  reject/accept reasons for each
- Step 3: Before locking in the leading conclusion, run **one more Serena verification query** intended to confirm or
  falsify it
- Step 4: If confidence is still below high, evidence conflicts, or the candidate set is thin/noisy, do **at least one
  more Serena lookup** with narrower or alternate constraints before acting or reporting
- If Serena returns fewer than 3 plausible candidates after widening once, record that the candidate space appears
  exhausted and proceed with the best verified option rather than pretending 3 exist
- Prefer Serena tools such as `search_for_pattern`, `find_symbol`, `get_symbols_overview`, and
  `find_referencing_symbols` before falling back to heavier reads
  </Serena_Decision_Protocol>
...
0.5 **Serena grounding pass for token-efficient accuracy**:
   - For non-trivial discovery, diagnosis, or implementation-choice work, run the `Serena_Decision_Protocol` before
     committing to a conclusion
   - Treat candidate as a plausible file, symbol, root cause, design path, or verification target
   - Use the extra verification lookup to challenge the current winner, not merely restate the same search
...
6. **Verify completion with fresh evidence**:
   ...
   e. If the change affects a server, API route, background worker, frontend runtime, browser flow, or any user-visible running system, add **runtime verification**:
      - Start the relevant service/app successfully
      - Confirm it stays up long enough to serve the target flow
      - Probe the changed path with the appropriate tool (`curl`, browser automation, app request, health check, etc.)
      - Treat "process started" as insufficient evidence; verify a real response or visible behavior
      - If the server/app does not start, crashes early, or cannot be reached, verification is **failed** -- do not claim completion
   f. For runtime-sensitive work, include the concrete command/output pair that proves the system is actually reachable
...
- If Serena MCP is available, prefer it as the primary read-optimized MCP for codebase discovery and decision support
- Use Serena for low-token candidate generation, comparison support, and extra pre-conclusion validation before broader
  shell/MCP escalation
- Use `ask_codex` with `agent_role: "architect"` for verification cross-checks when changes are security-sensitive,
...
- If ToolSearch finds no MCP tools or Codex is unavailable, proceed with architect agent verification alone -- never
...
instead of restarting or re-asking the same question.

**Good:** The user changes only the output shape or downstream delivery step (for example `make a PR`). Preserve earlier
non-conflicting workflow constraints and apply the update locally.

**Bad:** The user says `continue`, and the workflow restarts discovery or stops before the missing verification/evidence
is gathered.
...
<Bad>
Claiming completion after only build/test when the server path changed:
```
1. Run: ./gradlew test → PASS
2. Run: ./gradlew build → PASS
3. Report: "Done"
```
Why bad: If the task changed a running server/API/UI path, build/test alone is not enough. The service must actually start and respond.
</Bad>
...
- [ ] Serena candidate gathering/comparison/extra-validation loop completed for non-trivial lookups, or N/A with reason
- [ ] Zero pending or in_progress TODO items
- [ ] Fresh test run output shows all tests pass
- [ ] Fresh build output shows success
- [ ] Runtime verification completed for server/API/UI changes (or N/A with reason)
...
If `--no-deslop` is present, skip the deslop pass entirely after Step 7 and continue using the latest successful
pre-deslop verification evidence.
...
6. Guidelines: right-sized stories (one session each), verifiable criteria, independent stories, priority order (
...
Workflow: Detect flag, extract task, create `.omx/plans/prd-{slug}.md`, create `.omx/state/{scope}/ralph-progress.json`,
begin ralph loop.

### Legacy compatibility

- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from
  `.omx/prd.json`.
- `.omx/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup
  validation source.
- If `.omx/prd.json` exists and canonical PRD is absent, migrate one-way into `.omx/plans/prd-{slug}.md`.
- If `.omx/progress.txt` exists and canonical progress ledger is absent, import one-way into
