...
- [x] (2026-04-17 19:02+09:00) 작업 범위 조사 완료: `RewardPointGrantProcessor`, `CreateOrderService`,
  `BcryptMemberPasswordHasher`를 직접 생성 패턴 후보로 확정
...
- [x] 검증: 관련 테스트와 `./gradlew architectureLint`는 통과, `./gradlew check`는 `JpaMarketHistoryRepositoryTest` 스위트 실패로 blocker
...
- 관찰:
  ...
  증거:
  `rg -n "private final .* = new [A-Z]" backend/src/main/java` 결과로 `RewardPointGrantProcessor`, `CreateOrderService`,
  `BcryptMemberPasswordHasher`가 확인됐다.
...
- 관찰:
  ...
  의 `domain` 금지 규칙에 `Spring annotation`이 명시돼 있다.
...
- 관찰:
  ...
  에 `@Scheduled`와 `@PostConstruct`가 같이 있다.

- 관찰:
  `JpaMarketHistoryRepositoryTest`는 단독 실행과 일부 조합 실행에서는 통과하지만, 전체 `./gradlew check`에서는
  `assertEquals(1, count("market_candles_1m"))`에서 실패한다.
  증거:
...
그 다음 `RewardPointPolicy`와 `OrderPreviewPolicy`를 Spring 빈으로 등록하고, `RewardPointGrantProcessor`와 `CreateOrderService`는 생성자
주입으로 바꾼다.
