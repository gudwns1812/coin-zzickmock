...
- marketable 수정 정책은 `docs/exec-plans/active/open-order-edit-marketable-fill.md`에서 즉시 체결 지원으로 변경했다.
...
- 즉시 체결 가능한 가격 변경은 저장 직후 최신가 재조회 기준으로 같은 주문 row를 `FILLED`/`TAKER` 처리한다.

## 범위
- 이번에 하는 것(in scope): backend modify API/service/repository, frontend edit modal/button, 제품 명세 업데이트, 테스트/검증.
- 이번에 하지 않는 것(out of scope): 수량/레버리지/마진 모드 수정, TP/SL 수정, DB schema 변경.
- 후속 작업(선택): cancel-replace semantics.
...
- invalid/non-pending/conditional edit는 실패하고, marketable edit는 즉시 체결된다.
...
- 위험: marketable edit의 체결 semantics 혼선.
  - 예방: 같은 주문 row를 최신가 기준 `FILLED`/`TAKER`로 claim한다고 제품 명세에 기록.
  - 완화: open/close fill orchestration을 기존 체결 finalizer와 applier로 재사용.
  - 복구: guarded update/claim 실패 시 거절하고 다음 fresh market event로 deferred 처리.
...
3. After modify update, re-read fresh market; if the edited price became marketable, claim the same order row and apply taker fill accounting.
...
## Direct review after CodeRabbit rate limit
- [x] MEDIUM: Added a real Spring transaction/H2 rollback regression for post-update marketability rejection so the original pending order values are preserved after exception rollback.
- [x] LOW: Component-level DOM rendering test is not added because the current frontend test stack intentionally uses `node:test` without jsdom/RTL and the task forbids new dependencies. The extracted submit helper is behavior-tested for API call, close callback, refresh callback, success/error messages, and invalid-input blocking; populated modal click remains a runtime smoke gap.
- [x] HARDENING: Added underflowed positive `BigDecimal` validation so values that convert to `0.0` are rejected after the API boundary conversion.

## CodeRabbit follow-up review 4
- [x] MINOR: Replaced `Math.abs(UUID.randomUUID().getMostSignificantBits())` in the transaction test with a non-overflowing positive id expression.
