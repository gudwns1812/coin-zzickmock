...
- 거래량 보조 표시
- 현재 포지션과 미체결 지정가 주문을 가로 price line으로 표시
- candle-history 응답이 없을 때는 가짜 OHLC를 만들지 않고 실시간 가격선 셸로 fallback
...
- 예상 사용 증거금
- 예상 청산가
- preview CTA 없이 즉시 갱신되는 live risk summary
- 주문 버튼
...
- 열린 지정가 주문이 있으면 차트와 하단 목록에서 함께 확인하고 취소할 수 있다
