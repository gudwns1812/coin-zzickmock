...
| `1W`        | completed `market_candles_1h` 또는 `1D` 파생 결과 | 주 경계 롤업   |
| `1M`        | completed `market_candles_1h` 또는 `1D` 파생 결과 | 월 경계 롤업   |

핵심은 "사용자 지원 기간 수"와 "DB 물리 테이블 수"를 1:1로 맞추지 않는 것이다.
현재 기준에서는 `1m`, `1h`만 물리 저장하고, 나머지는 애플리케이션 계층에서 파생하는 방식을 기본값으로 둔다.

REST history에서 `1h`는 저장된 raw hourly row가 존재한다는 이유만으로 완료된 봉으로 보지 않는다.
해당 hour의 `[hourOpen, hourClose)` 구간에 기대되는 모든 `1m` open time이 연속으로 존재해야 REST-visible completed `1h`로 간주한다.
단순 row count, source minute min/max, 또는 첫/마지막 minute 존재만으로는 completed 조건을 만족한 것으로 보지 않는다.
`3m`, `5m`, `15m`은 별도 저장 테이블이나 독립 completion 검사를 두지 않고, 저장 완료된 `1m` 이벤트를 기준으로 프론트엔드 REST history query를 무효화한다.
`4h`, `12h`, `1D`, `1W`, `1M`도 raw partial `market_candles_1h`를 직접 읽지 않고 completed `1h` 입력만 rollup해야 한다.
...
- 기존 live candle bucket 전환 후 지연 refetch는 fallback으로 유지할 수 있지만, 저장 완료 반영의 주된 정확성 경로는 `historyFinalized` 메시지여야 한다.
...
- `1h` REST 조회는 contiguous `1m` coverage가 증명된 completed hourly row만 반환한다.
...
- [backend/src/main/resources/db/migration/V3\_\_add_market_history_schema.sql](/Users/hj.park/projects/coin-zzickmock/backend/src/main/resources/db/migration/V3__add_market_history_schema.sql)
