...
- `docker-compose.backend.prod.yml`: backend app, push-app, public Nginx, backend-host Scouter collector, backend-host promtail, nginx exporter, backend node exporter만 소유한다. Redis, Prometheus, Grafana, Loki는 포함하지 않는다.
...
- `backend_image`: `backend/**` 변경 또는 수동 `backend-image`. Backend check를 실행하고 Docker Hub에 새 backend/push-app/Scouter collector ARM64 image를 push한 뒤, backend host `.env.prod`의 `BACKEND_IMAGE`, `PUSH_IMAGE`, `SCOUTER_COLLECTOR_IMAGE`를 새 태그로 바꾸고 Scouter collector/backend/push-app만 pull/restart한다. Infra host는 건드리지 않는다.
- `backend_runtime`: `docker-compose.backend.prod.yml` 변경 또는 수동 `backend-runtime`. 새 backend image를 만들지 않고 backend host compose/env preflight 후 backend/push-app runtime을 recreate한다. Split compose migration 중 이전 `coin-zzickmock` backend container가 backend host port를 잡고 있으면 해당 expected stale backend container만 제거한 뒤 새 backend와 push-app을 올릴 수 있다. Infra host는 건드리지 않는다.
- `backend_agent_runtime`: `infra/promtail/**` 또는 backend-host telemetry agent service definition 변경. Backend app image를 만들거나 backend app을 recreate하지 않고 backend-host Scouter collector/agent/exporter만 반영한다.
  Backend-host agent deploy may remove stale containers that already hold the exporter host ports `9100` or `9113`, but only when the holder is an expected node-exporter or nginx-exporter container. If another container owns those ports, CD fails instead of stopping an unknown service.
...
- `nginx_config`: `infra/nginx/**` 변경 또는 수동 `nginx-config`. 새 backend image를 만들지 않고 backend host Nginx config만 sync/test/reload한다. Nginx service가 내려가 있으면 config 검증 전에 Nginx만 기동한다. 이때 80/443을 잡고 있는 이전 `coin-zzickmock` Nginx 컨테이너는 새 compose Nginx 기동을 위해 제거할 수 있지만, 다른 컨테이너가 포트를 잡고 있으면 실패한다.
...
2. `verify_backend_candidate`는 항상 job graph에 존재하지만, `backend_image=true`일 때만 backend `./gradlew check :app:bootJar :push-app:bootJar`를 실행하고 executable jars를 workflow artifact로 업로드한다.
3. `publish_backend_image`도 항상 job graph에 존재하지만, `backend_image=true`일 때만 jar artifact를 `backend/app/build/libs/`와 `backend/push-app/build/libs/`에 내려받아 backend, push-app, Scouter collector runtime Docker images로 포장한 뒤 Docker Hub login/build/push를 실행한다.
...
7. `backend_image=true`일 때만 backend host `.env.prod BACKEND_IMAGE`, `PUSH_IMAGE`, `SCOUTER_COLLECTOR_IMAGE`를 candidate image로 교체한다. 다른 runtime 배포는 `.env.prod` 비밀값이나 image tag를 바꾸지 않는다.
...
- `<DOCKERHUB_USERNAME>/coin-zzickmock-push-app:<tag>`
- `<DOCKERHUB_USERNAME>/coin-zzickmock-scouter-collector:<tag>`

운영 EC2는 Amazon Linux `aarch64` 기준이므로 backend, push-app, Scouter collector 이미지는 `linux/arm64` 플랫폼으로 build/push한다.
...
CD는 Dockerfile의 `jar-runtime`/`push-jar-runtime`/`scouter-collector-runtime` target을 사용해 `backend/app/build/libs/app.jar`와 `backend/push-app/build/libs/push-app.jar`를 runtime image에 복사하고 Scouter collector image를 포장한다. Backend/push-app Docker build 안에서는 Gradle을 실행하지 않는다.
로컬 Compose는 개발 편의를 위해 `source-runtime`/`push-source-runtime` target을 사용하고 Docker build 안에서 source jar를 만든다.
...
`.env.prod`는 Git에 커밋하지 않는다. CD가 backend host `.env.prod`에서 수정할 수 있는 값은 `^BACKEND_IMAGE=`, `^PUSH_IMAGE=`, `^SCOUTER_COLLECTOR_IMAGE=`뿐이며, 다른 runtime 비밀값은 서버 소유 상태로 유지한다.
...
- `SCOUTER_COLLECTOR_IMAGE`: 현재 운영 Scouter collector image. 예: `dockerhub-user/coin-zzickmock-scouter-collector:<tag>`
...
- `PUSH_BIND_ADDRESS`: 선택값. push-app host port bind 주소이며 기본은 `0.0.0.0`이다. infra host 외 public 노출은 security group으로 막는다. Nginx container는 Docker network `push-app:8081`로 접근한다
- `SCOUTER_BIND_ADDRESS`: 선택값. Scouter collector host port bind 주소이며 기본은 `127.0.0.1`이다. 외부 operator 접속이 필요하면 public이 아닌 private/operator-restricted 주소로만 바꾼다
- `SCOUTER_WEB_PORT`, `SCOUTER_COLLECTOR_PORT`: 선택값. Scouter HTTP/API port와 collector tcp/udp port이며 기본은 `6180`, `6100`이다
...
운영 backend compose는 Spring Boot actuator metrics, push-app actuator metrics, backend-host exporter metrics를 별도 infra host Prometheus가 가져갈 수 있도록 backend host에 publish한다. Spring Boot actuator와 push-app actuator는 direct/private `8080`/`8081`로 확인하고, backend-host exporters는 cloud 보안그룹에서 `9100/9113` high port가 닫혀도 동작하도록 backend Nginx의 private relay path를 통해 확인한다.

- Backend application scrape URL: `http://<EC2_BACKEND_METRICS_HOST>:8080/actuator/prometheus`
- Push-app scrape URL: `http://<EC2_BACKEND_METRICS_HOST>:8081/actuator/prometheus`
...
- Prometheus config: `infra/prometheus/prometheus.prod.yml`은 `backend-private:8080`, `backend-private:8081`, `backend-private:80/internal/metrics/*`를 scrape하며, infra compose의 `extra_hosts`가 `backend-private`을 `BACKEND_PRIVATE_HOST`로 해석한다.
- CD 검증: backend-host deploy 후 GitHub Actions가 `INFRA_EC2_HOST`에 SSH로 접속해 위 네 scrape URL을 `curl -fsS`로 확인한다. 검증은 `https://.../actuator/*` 또는 Nginx public domain을 사용하지 않는다.
- Network rule: backend host security group은 TCP 8080/8081 inbound를 infra host source로 제한해야 한다. Exporter relay는 backend Nginx tcp 80의 `/internal/metrics/*` location에서 private CIDR만 허용하고 public source는 `deny all`로 막는다. 이 repository는 security group을 직접 변경하지 않는다.

## Deploy Scope

수동 실행의 `deploy_scope`는 아래 값을 사용한다.

- `backend-image`: 새 backend/push-app/Scouter collector image를 build/push하고 backend host `.env.prod BACKEND_IMAGE`/`PUSH_IMAGE`/`SCOUTER_COLLECTOR_IMAGE`를 교체한 뒤 Scouter collector, backend, push-app을 pull/restart한다.
- `backend-runtime`: 새 image 없이 backend host compose/runtime 설정만 적용하고 backend/push-app runtime을 recreate한다.
- `backend-agent-runtime`: backend-host Scouter collector/promtail/nginx-exporter/node-exporter runtime만 적용한다. Backend app은 recreate하지 않는다.
...
Backend 또는 push-app recreate 후 health가 실패하면 workflow는 실패한다. 기본 정책은 automatic rollback 없음이다. 이미 시도된 backend/push/Scouter collector image/env/runtime 상태를 그대로 두고, 운영자가 릴리즈 기록과 [03-rollout-and-rollback.md](03-rollout-and-rollback.md) 기준으로 수동 롤백 여부를 판단한다.
