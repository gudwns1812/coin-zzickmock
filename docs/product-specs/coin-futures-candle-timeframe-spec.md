...
현재 저장소의 시장 히스토리 스키마는 `market_candles_1m` 원본 분봉과 `market_completed_candles` completed 캔들 테이블을 가진다.
`market_completed_candles`는 API 라벨이 아니라 DB 안전 토큰인 `ONE_HOUR`, `ONE_DAY`, `ONE_MONTH`를 `candle_interval`에 저장해 `1h`, `1D`, `1M` 완료 봉을 구분한다.
하지만 사용자 경험 관점에서는 저장된 completed interval만으로 충분하지 않다.
...
- DB에 물리 저장되는 캔들 기준은 `market_candles_1m` 원본 분봉과 `market_completed_candles` completed 캔들이다.
- `1m`는 원본 분봉 저장 기준이다.
- `1h`, `1D`, `1M`은 `market_completed_candles.candle_interval`에 각각 `ONE_HOUR`, `ONE_DAY`, `ONE_MONTH` 토큰으로 저장되는 REST-visible completed 캔들이다. API 라벨 `1h`/`1D`/`1M`은 저장 토큰과 분리한다.
- API와 프론트 차트는 `1m`, `3m`, `5m`, `15m`, `1h`, `4h`, `12h`, `1D`, `1W`, `1M`을 지원 기간으로 고정한다.
- `3m`, `5m`, `15m`, `4h`, `12h`, `1W`는 별도 물리 테이블을 기본값으로 두지 않고 저장된 `1m` 또는 completed `1h` 캔들에서 파생한다.
- `1D`, `1M`은 completed `1h` 입력의 완전성이 증명된 후 `market_completed_candles`에 물리 저장하고, REST 기본 조회는 이 persisted row를 직접 읽는다.

즉, 저장 구조는 "표시 가능한 전체 기간 목록"이 아니라 "원본 분봉 + 자주 쓰는 completed 캔들 + 나머지 파생 기간" 기준이다.
...
- `1h`, `1D`, `1M`은 `market_completed_candles`의 completed interval로 유지한다.
- `market_completed_candles.candle_interval`은 `ONE_HOUR`, `ONE_DAY`, `ONE_MONTH`처럼 대소문자와 API 라벨 충돌이 없는 DB token을 사용한다.
- `market_completed_candles.source_interval`은 `ONE_HOUR` row에는 `ONE_MINUTE`, `ONE_DAY`/`ONE_MONTH` row에는 `ONE_HOUR`를 저장한다.
- `source_candle_count`는 저장 시점에 증명한 원천 row 수를 기록한다. `ONE_HOUR`는 60, `ONE_DAY`는 24, `ONE_MONTH`는 UTC 월 경계의 실제 hour 수다.
...
| `4h`        | completed `ONE_HOUR` rows               | 고정 4-row window 롤업 |
| `12h`       | completed `ONE_HOUR` rows               | 고정 12-row window 롤업 |
| `1D`        | `market_completed_candles` (`ONE_DAY`)  | 직접 조회      |
| `1W`        | completed `ONE_HOUR` rows               | 주 경계 롤업   |
| `1M`        | `market_completed_candles` (`ONE_MONTH`)| 직접 조회      |

핵심은 "사용자 지원 기간 수"와 "DB 물리 테이블 수"를 1:1로 맞추지 않는 것이다.
현재 기준에서는 `1m` 원본과 completed `1h`/`1D`/`1M`만 물리 저장하고, `3m`/`5m`/`15m`/`4h`/`12h`/`1W`는 애플리케이션 계층에서 파생하는 방식을 기본값으로 둔다.

REST history에서 `1h`, `1D`, `1M`은 `market_completed_candles`에 저장된 REST-visible completed row를 직접 조회한다.
...
`market_completed_candles`의 `ONE_HOUR` row는 저장 또는 재빌드 시점에 해당 hour의 `[hourOpen, hourClose)` 구간에 기대되는 모든 `1m` open time 60개가 연속으로 존재할 때만 저장된다.
...
`4h`, `12h`, `1W`는 partial 후보가 아닌 completed `ONE_HOUR` 입력만 rollup해야 한다.
`4h`와 `12h`는 이미 정렬되고 UTC 경계에 맞춰 조회된 completed `ONE_HOUR` row를 각각 4개 또는 12개 fixed window로 소비한다. window 안의 첫 hour, 중간 hour, 마지막 hour가 모두 기대 open time과 일치할 때만 파생 candle을 반환한다.
`1D`와 `1M`은 `market_completed_candles`의 `ONE_DAY`, `ONE_MONTH` row를 직접 읽는다. 이 row는 UTC 일 경계 `[00:00, 다음 00:00)` 또는 UTC 월 경계 `[1일 00:00, 다음 달 1일 00:00)` 안의 모든 completed `ONE_HOUR` row가 저장 또는 재빌드 시점에 증명된 경우에만 upsert한다.
`1W`는 계속 UTC ISO 주 경계 `[월요일 00:00, 다음 월요일 00:00)` 안의 모든 completed `ONE_HOUR` row가 있을 때만 파생해 반환한다.
하나의 completed `ONE_HOUR` row라도 누락되면 상위 candle은 제외하며, 기대 hour 수는 calendar boundary로 계산한다.
...
월/주 단위 completeness 검사가 hot path 비용이 되면 `market_completed_candles(candle_interval, open_time, symbol_id)` 접근 경로를 유지한 상태에서 materialized completeness table이나 hourly coverage bitmap 같은 계산된 metadata를 도입해 매 요청마다 전체 hourly row를 스캔하지 않게 한다.

### 후속 확장 원칙

특정 기간의 조회량이 커지거나 롤업 비용이 부담되면, 그때 `4h`, `12h`, `1W` 같은 중간 interval을 `market_completed_candles`에 추가 물리화할 수 있다.
...
`1D`, `1M`처럼 직접 저장되는 calendar interval은 `market_completed_candles`의 최신 completed row를 기준으로 DB 조회 범위를 잡고, `1W`처럼 `1h`에서 파생하는 기간은 최신 완료 주간 버킷을 기준으로 DB 조회 범위를 잡는다.
...
- REST `1h`, `1D`, `1M`은 persisted completed row를 직접 읽고, `4h`, `12h`, `1W`는 completed `ONE_HOUR` 입력만 사용해야 하며, 현재 진행 중인 partial hourly row를 응답에 포함하면 안 된다.
- 자동 calendar catch-up/backfill은 해당 심볼+calendar interval에 기존 persisted row가 하나 이상 있을 때만 후속 닫힌 버킷을 보정한다. 최초 전체 과거 `1D`/`1M` seed는 startup 또는 일반 REST read에서 자동으로 수행하지 않고, 별도 offline/admin 작업으로만 다룬다.
...
- `1m`은 DB 원본 분봉에서 직접 조회되고, `1h`, `1D`, `1M`은 `market_completed_candles`의 persisted completed row에서 직접 조회된다.
- `1h` REST 조회는 저장 시점에 contiguous `1m` coverage가 증명된 completed hourly row만 반환하며 조회 시점에 `1m` coverage를 다시 스캔하지 않는다.
- `1D`, `1M` REST 조회는 저장 시점에 해당 UTC day/month의 completed `ONE_HOUR` coverage가 증명된 persisted row만 반환한다.
- `3m`, `5m`, `15m`, `4h`, `12h`, `1W`은 현재 저장 구조를 기준으로 올바르게 파생되어 응답된다.
- `4h`, `12h`, `1W`은 completed `ONE_HOUR` 입력만 rollup한다.
