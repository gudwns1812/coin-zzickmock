...
- completed 캔들은 `market_completed_candles`에 저장하며, `candle_interval` DB token으로 `ONE_HOUR`, `ONE_DAY`, `ONE_MONTH`를 구분한다. API label은 계속 `1h`, `1D`, `1M`이다.
- `ONE_HOUR` row는 `source_interval=ONE_MINUTE`, `source_candle_count=60`을 기록한다. `ONE_DAY`/`ONE_MONTH` row는 completed `ONE_HOUR`를 source로 하는 calendar completed row이며, 저장 시 검증된 source hour 수를 `source_candle_count`에 기록한다.
- API와 프론트 차트는 `1m`, `3m`, `5m`, `15m`, `1h`, `4h`, `12h`, `1D`, `1W`, `1M`을 지원 기간으로 고정한다.
- `3m`, `5m`, `15m`, `4h`, `12h`, `1W`는 별도 물리 테이블을 기본값으로 두지 않고 저장된 `1m`/completed `1h` 캔들에서 파생한다.
- `1D`, `1M` REST DB read는 `market_completed_candles`의 `ONE_DAY`/`ONE_MONTH` row를 직접 조회한다. 일반 REST read path에서 매 요청마다 completed `1h`를 calendar rollup하지 않는다.
...
| `1W`        | `market_completed_candles` (`ONE_HOUR`)  | 주 경계 롤업   |
...
REST history에서 `1h`는 `market_completed_candles(candle_interval=ONE_HOUR)`에 저장된 REST-visible completed row를 직접 조회한다. `1D`와 `1M`도 각각 `ONE_DAY`, `ONE_MONTH` completed row를 직접 조회한다.
...
`4h`, `12h`, `1W`는 partial 후보가 아닌 completed `1h` 입력만 rollup해야 한다. `1D`, `1M`은 completed calendar row를 직접 읽으며, 저장/재빌드 경로에서만 completed `1h` coverage를 검증한다.
Completed calendar candle에서 `1D`는 UTC 일 경계 `[00:00, 다음 00:00)`, `1W`는 UTC ISO 주 경계 `[월요일 00:00, 다음 월요일 00:00)`, `1M`은 UTC 월 경계 `[1일 00:00, 다음 달 1일 00:00)`를 따른다. `1W`는 요청 시 completed `1h`에서 파생하고, `1D`/`1M`은 저장 시점에 해당 경계 안의 모든 completed `1h` row가 확인된 경우에만 `market_completed_candles`에 남긴다.
...
월/주 단위 completeness 검사가 hot path 비용이 되면 `market_completed_candles(candle_interval, open_time, symbol_id)` 인덱스 또는 별도 materialized completeness table/hourly coverage bitmap 같은 계산된 metadata를 도입해 매 요청마다 전체 hourly row를 스캔하지 않게 한다.
...
`1W`처럼 `1h`에서 파생하는 기간은 최신 저장 `1h`가 아니라 최신 완료 주 버킷을 기준으로 DB 조회 범위를 잡는다. `1D`/`1M`은 persisted completed calendar row의 최신 open time을 interval별 cursor 기준으로 사용한다.
...
- `1m`, `1h`, `1D`, `1M`은 DB에 저장된 원본 또는 completed 캔들 테이블에서 직접 조회된다.
- `1h` REST 조회는 저장 시점에 contiguous `1m` coverage가 증명된 completed hourly row만 반환하며 조회 시점에 `1m` coverage를 다시 스캔하지 않는다.
- `3m`, `5m`, `15m`, `4h`, `12h`, `1W`는 현재 저장 구조를 기준으로 올바르게 파생되어 응답되고, `1D`/`1M`은 persisted completed calendar row를 기준으로 응답된다.
- `4h`, `12h`, `1W`는 completed `1h` 입력만 rollup한다. `1D`, `1M` 저장/재빌드는 completed `1h` 입력의 완전성을 증명한 뒤 persisted row를 upsert한다.
