...
로그인/로그아웃 같은 auth change 이벤트는 `components/router/QueryClientProvider.tsx`에서 개인화 React Query cache를 제거합니다. 개인화 mutation은 `lib/futures-query-invalidation.ts`의 query invalidation helper를 사용하고, 서버 컴포넌트 refresh를 개인화 데이터 재조회 수단으로 의존하지 않습니다.
