...
- 로컬 HTTP Compose 검증에서는 browser가 backend cookie를 저장할 수 있도록 `APP_AUTH_COOKIE_SECURE=false`, `APP_AUTH_COOKIE_SAME_SITE=Lax` 기본값을 사용한다. Production에서는 `docker-compose.backend.prod.yml`/backend host `.env.prod` 계약이 `true`/`None`을 유지해야 한다.
