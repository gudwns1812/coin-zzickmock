...
- `nginx_config`: `infra/nginx/**` 변경 또는 수동 `nginx-config`. 새 backend image를 만들지 않고 backend host Nginx config만 sync/test/reload한다. Nginx service가 내려가 있으면 config 검증 전에 Nginx만 기동한다.
