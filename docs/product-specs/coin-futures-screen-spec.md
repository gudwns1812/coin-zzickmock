...
사용자가 현재 거래 가능한 두 심볼을 빠르게 비교하고, 바로 상세 화면으로 이동하거나 관심 목록에 넣게 만든다.
...
- watchlist가 비어 있으면 "관심 심볼 추가" 안내 카드 표시
- 포지션이 없으면 "첫 포지션 열기" CTA 표시

### 오류 상태

- Bitget 시세 수집 실패 시 읽기 전용 경고 배너
- stale data일 때 마지막 업데이트 시간 표시

### 수용 기준

- 두 심볼이 모두 보인다
- 가격, 등락률, funding rate를 한 화면에서 비교할 수 있다
- 관심 심볼과 포지션 요약이 동시에 보인다
