...
- Redis ZSET 같은 파생 읽기 모델은 서비스가 기대하는 rank score, fallback, 관측 계약을 테스트로 고정한다. 특히 DB fallback의 내 순위가 tie-break 없이 "나보다 score가 높은 멤버 수 + 1"로 계산되는지, 잘못된 limit이 전체 Redis range 조회로 이어지지 않는지, 누락된 hash가 조용히 부분 응답으로 노출되지 않는지 검증한다.
...
- feature `application/command`, `application/result`, `application/realtime` package는 migration residue 재도입으로 간주해 `APPLICATION_CONTEXT_PACKAGE_RESIDUE_FORBIDDEN` 규칙으로 차단
...
Core split ratchet은 기존 `web/job` ratchet과 같은 원칙을 따른다. 첫 slice는 기존 app application/domain residue를 구조화 allowlist로 기록할 수 있지만, 새 residue는 실패한다. feature 영역을 만지는 slice는 해당 영역의 residue count를 줄이거나 allowlist 제거 근거를 남겨야 한다.
