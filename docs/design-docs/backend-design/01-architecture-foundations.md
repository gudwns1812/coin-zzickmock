...
로그인 사용자의 내 순위는 Redis snapshot version을 한 번 읽은 뒤 해당 `profitRate` ZSET에서 `ZREVRANK + 1`로 계산한다. 내 순위 계산은 member hash hydrate를 요구하지 않는다.
Redis를 사용할 수 없는 경우 DB fallback은 `profitRate DESC, memberId ASC` 순서로 단일 순위를 계산한다. 이 동률 순서는 Redis ZSET의 exact tie ordering과 완전 동일하다고 보장하지 않는다.
