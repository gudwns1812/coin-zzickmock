...
- DB 변경은 현재 `backend/app/src/main/resources/db/migration` 아래 새 Flyway migration과 `docs/generated/db-schema.md` 갱신을 함께 한다. 후속 storage module 이동 후에는 해당 governing document를 먼저 갱신한다.
