...
export async function GET(request: Request) {
  ...
  const upstreamParams = new URLSearchParams({ symbols: symbols.join(","), [SSE_CLIENT_KEY_PARAM]: clientKey });
  ...
  return proxySseStream({
    ...
    upstreamUrl: `${FUTURES_API_BASE_URL}/api/futures/stream/markets/summary?${upstreamParams.toString()}`,
  });
}
