...
- Planned moves:
  ...
  - `RealtimeMarketCandleUpdate`, `RealtimeMarketTickerUpdate`, `RealtimeMarketTradeTick`, `MarketRealtimeSourceSnapshot`, `MarketHistoryPersistenceResult`, `MarketHistoryPersistenceStatus`, `MarketRealtimeHealth`, `MarketRealtimeSourceType` -> `market/application/dto`.
  - `MarketPriceMovementDirection`, `RealtimeMarketCandleState`는 PR-0A에서 `dto`/`domain`/`implement` 중 확정한 target 없이는 PR-9 실행 금지.
...
- Exact planned moves:
  ...
  | `MarketPriceMovementDirection` | `market/application/dto` unless PR-0A proves domain rule |
  | `RealtimeMarketCandleState` | execution blocked until PR-0A decides `dto` vs `implement` |
...
- Exact planned moves:
  | Source | Target |
  | --- | --- |
  | `DelayedClosedMinuteCandlePersistenceScheduler` | split: scheduling trigger/config in `app/feature/market/job`; core delayed persistence coordinator in `market/application/history` or `market/application/implement` |
  | `MarketHistoryStartupBackfill` | `market/application/repair` or `market/application/history`; startup trigger remains in `app/feature/market/job` |
  | `CurrentMarketCandleBootstrapper` | split if trigger exists: trigger in `app/feature/market/job`; bootstrap coordinator in `market/application/history` or `market/application/implement` |
- Tests:
  ```bash
  cd backend
  ./gradlew test --tests '*MarketHistoryStartupBackfillTest' --tests '*CurrentMarketCandleBootstrapperTest' --tests '*MarketStartupReadyEventIntegrationTest' --tests '*MarketHistoryPersistenceCoordinatorTest' --tests '*MarketSchedulingConfigurationTest' --tests '*MarketRealtimeRefreshSchedulerTest' --tests '*MarketHistoryStartupBackfillReadyEventListenerTest' --tests '*MarketMinuteBoundaryEventPublisherTest' --console=plain
...
- Exact planned moves:
  ...
  | `MarketHistoryPersistenceCoordinator` | `market/application/history` unless PR-0A proves public service entrypoint |
  | `MarketHistoryRecorder` | `market/application/history` unless PR-0A proves public service entrypoint |
  | `MarketMinuteCandleHistoryListener` | `market/application/history` or `market/application/service` if event entrypoint is public use case |
  | `MarketRealtimeFreshnessPolicy` | `market/application/implement` or domain only if PR-0A proves product rule |
  | `MarketRealtimeReconnectState` | `market/application/implement` |
  | `MarketSnapshotStore` | `market/application/implement` |
  | `MarketSupportedMarketRefresher` | `market/application/implement` unless PR-0A proves public refresh use case |
